...
const AdminDashboard = () => {
  ...
  const fetchOrderStats = async () => {
    try {
      ...
      const registrationResponse = await fetch(`${getApiUrl('admin/get_registrations.php')}?limit=1`);
      ...
      if (data.success) {
        ...
        const registrationCount = registrationData.success ? (registrationData.data?.pagination?.total_records || 0) : 0;
    ...
    <div className="min-h-screen bg-gray-600 flex">
      ...
      <main className="flex-1 p-8">
        ...
        <div className="space-y-6">
          ...
          {/* Second Row - Blog Management and Registration Leads */}
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <div 
              className="bg-gray-800 border border-gray-700 rounded-lg p-6 hover:bg-gray-750 transition-colors cursor-pointer"
              onClick={() => navigate('/admin/blog')}
            ...
            >
              <div className="flex items-center justify-between mb-4">
                ...
                <span className="text-2xl font-bold text-white">{orderStats.registrationCount || 0}</span>
              </div>
              <h3 className="text-lg font-semibold text-white mb-2">Registration Leads</h3>
              <p className="text-gray-400 text-sm">Manage user registrations</p>
              <div className="mt-4 flex items-center text-purple-400">
                <svg className="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                </svg>
                <span className="text-sm">Click to manage leads</span>
              </div>
            </div>
          </div>
        </div>
